export default [
  {
    ...
    issue:
      'Above-the-fold images using the `priority` prop still suffer from suboptimal Largest Contentful Paint (LCP) scores.',
  ...
  },
  {
    ...
    issue:
      'A specific combination of transform streams can cause an unhandled rejection error when the pipeline is abruptly closed.',
    rootCause:
      'The destroy callback was not properly propagating errors up the pipeline chain in half-closed streams.',
  ...
  },
  {
    ...
    issue:
      'A subtle memory leak occurs when concurrent renders interrupt a mount sequence, leaving stale event listeners.',
    rootCause:
      'The cleanup function was only considering synchronous unmounts, missing the interrupted state.',
  ...
  },
  {
    ...
    tags: ['bug-fix', 'css'],
    issue:
      'Dynamically constructed class names in the safelist array are not being generated in the final CSS bundle.',
    rootCause:
      'The regex parser was skipping safelist entries that used string interpolation without evaluating them.',
  ...
  },
  {
    ...
    issue:
      'TypeScript fails to infer the correct generic type when passing deeply nested object literals to a utility function.',
  ...
  },
  {
    ...
    issue:
      'Opening files larger than 50MB causes the extension host to crash due to OOM when semantic highlighting is enabled.',
    rootCause:
      'The tokenizer was buffering the entire file content into a single string before processing.',
